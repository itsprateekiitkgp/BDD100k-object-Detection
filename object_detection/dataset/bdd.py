...
def load_images_and_anns(im_sets, label2idx, ann_fname, split):
    ...
    for im_set in im_sets:
        ...
        ann_file = os.path.join('data/labels', f'bdd100k_labels_images_{split}.json')
        im_dir = os.path.join(im_set, f'{split}')
        ...
        for item in data:
            im_info = {}
            im_info['filename'] = os.path.join(im_dir, item['name'])
            im_info['width'] = 1280
            im_info['height'] = 720
            detections = []
            for label in item.get("labels", []):
                cat = label.get("category")
                if cat in label2idx.keys() and "box2d" in label:
                    det = {}
                    difficult = label['attributes']['occluded']
                    box = label["box2d"]
                    bbox = [box["x1"], box["y1"], box["x2"], box["y2"]]
                    det['label'] = label2idx[cat]
                    det['bbox'] = bbox
...
class BDDDataset(Dataset):
    def __init__(self, split, im_sets, im_size=640):
        ...
        classes = [
            'car', 'truck', 'bus', 'train', 'person', 'rider', 'motorcycle', 'bicycle', 'traffic light', 'traffic sign'
        ]
